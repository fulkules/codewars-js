...
// fn(str, num) => str
// early return if sz is > string length or is 0 or less
// separate the strings into chunks of length sz
// if sum the cubes of each of the digits in the chunk are divisible by 2
    // reverse the chunk
// else
...
//
//     let test = s => Array.prototype.reduce.call(s, (acc, cur) => acc + Number(cur) ** 3, 0) % 2 === 0;
//     let reverse = s => s.split("").reverse().join("");
//     let rotate = s => s.slice(1) + s.slice(0, 1);
//
//     let arr = [];
//     for(let i = 0; i < str.length; i += sz) arr.push(i + sz <= str.length ? str.slice(i, i + sz) : "");
