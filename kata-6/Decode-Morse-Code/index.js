...
const decode = (morseCode) => {
    // initialiaze an empty string variable
    let decodeString = '';
    // split the moresCode at the 3 spaces into an array of words
    let morseCodeWords = morseCode.split('   ');
    // iterate the array of words
    for (let i in morseCodeWords) {
        // split the letters at 1 space into array of chars
      let morseCodeArray = morseCodeWords[i].split(' ');
      // iterate the characters
      for (let j in morseCodeArray) {
          // if the char at this idx is not undefined
        if (MORSE_CODE[morseCodeArray[j]] !== undefined) {
            // concatenate this char to the initialized empty string
          decodeString += MORSE_CODE[morseCodeArray[j]];
        }
      }
      // concat a space after each word
      decodeString += ' ';
    }
    // trim whitespace from the new string and convert to uppercase
    decodeString = decodeString.trim().toUpperCase();  
