...
// need to go one letter at a time in order
    // iterate the str
// put a new string with the Uppercase letter at each position into an array
// return the array
...
function wave(str){
    ...
    for(let i = 0; i < str.length; i++){
        if(str.charAt(i) !== ' '){
            arr.push(`${str.slice(0, i)}${str.charAt(i).toUpperCase()}${str.slice(i + 1, str.length)}`);
        }
    }
    return arr;
}
