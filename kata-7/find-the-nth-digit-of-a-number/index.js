// Complete the function that takes two numbers as input,
//     num and nth and return the nth digit of num (counting from right to left).
//
// Note
// If num is negative, ignore its sign and treat it as a positive value
// If nth is not positive, return -1
// Keep in mind that 42 = 00042. This means that findDigit(42, 5) would return 0
// Examples
// findDigit(5673, 4)     returns 5
// findDigit(129, 2)      returns 2
// findDigit(-2825, 3)    returns 8
// findDigit(-456, 4)     returns 0
// findDigit(0, 20)       returns 0
// findDigit(65, 0)       returns -1
// findDigit(24, -8)      returns -1

// fn(num, num) => num
// if nth < 1 return -1
// convert num to string
// if str.length < nth return 0
// reverse and slice the string at nth
// convert it to num
// return num

const findDigit = (num, nth) => {
    if(nth < 1) return -1;
    let numStr = Math.abs(num).toString().split('').reverse().join('');
    if(numStr.length < nth) return 0;
    let digit = numStr.charAt(nth-1);
    return Number(digit);
};

// alternate solution
// const findDigit = function(num, nth){
//     let arr = Array.from(Math.abs(num).toString()).reverse();
//     if(arr.length < nth) return 0;
//     if(nth < 1) return -1;
//     return +Array.from(num.toString()).reverse()[nth-1]
// }

module.exports = findDigit;