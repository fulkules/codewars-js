...
// average speed per hour obtained on the sections of x. If x length is
// less than or equal to 1 return 0 since the car didn't move.
//
// Example:
...
// fn(arr) => int
// iterate array
// early return 0 if x.length <= 1
// get the distance between each num in array
// calculate the average hourly speed on every section
// get the floor of the max avg speed

function gps(s,x){
    if(x.length <= 1) return 0;
    let speeds = [];
    for(let i = 1; i < x.length; i++){
        speeds.push((3600 * (x[i] - x[i - 1])) / s);
    }
