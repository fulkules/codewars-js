...
//     An example would be (where the first term is 0 and the
// constant is 1) is [0, 1, 2, 3, 4, 5, ... and so on] )
// Else if (pun) f(n) = f(n-1) * c where c is a constant value,
//     then f is a geometric sequence.
//     Example where the first term is 2 and the constant is 2
// will be [2, 4, 8, 16, 32, 64, ... to infinity ... ]
...
// to move to the next element in the "set").
//
...
// fn(num, num, num) => num
// initialize an array with the first arg as value at i = 0
// build the array from the remaining arguments
    // iterate up to second argument
    // add third arg to the prev idx value
// return the array at the idx = second arg
